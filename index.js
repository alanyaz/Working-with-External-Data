...
async function handleBreedSelection() {
  ...
  try {
    ...
    // Append the paragraph to the infoDump
    infoDump.innerHTML = `<h2>Information about ${breedSelect.options[breedSelect.selectedIndex].textContent}</h2>`;
...
// Attach the handleBreedSelection function to the change event of breedSelect
...
async function initialLoad() {
  try {
    // Use axios to fetch the list of cat breeds from the cat API
...
async function handleBreedSelection() {
  ...
  try {
		...
		Carousel.clear();
		infoDump.innerHTML = '';
    // Use axios to fetch information on the selected breed from the cat API
